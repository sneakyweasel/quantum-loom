...
//--------------------------INIT------------------------------------------------
var idGate = [["B", 3], ["R", -2], ["B", -4], ["R", 2], ["B", 4], ["R", 2], ["B", -2], ["R", -2], ["B", -4], ["R", -4], ["B", -2], ["R", 4], ["B", 2], ["R", -2], ["B", 2], ["R", 2], ["B", -2], ["R", 3]];
var notGate = [["R", -2], ["B", -4], ["R", 4], ["B", -2], ["R", 2], ["B", 2], ["R", -2], ["B", 4], ["R", -2], ["B", 4], ["R", -2], ["B", 4], ["R", 2], ["B", -4], ["R", 2], ["B", -2], ["R", 2], ["B", -2], ["R", -2]];
...
var ellipses = [{ cx: -margin.left / 2, cy: 3, rx: 1, ry: 1.4 }, { cx: -margin.left / 2, cy: 0.5, rx: 0.8, ry: 0.9 }, { cx: -margin.left / 2, cy: 3.5, rx: 0.8, ry: 0.9 }, { cx: width + margin.right / 2, cy: 3, rx: 1, ry: 1.4 }, { cx: width + margin.right / 2, cy: 0.5, rx: 0.8, ry: 0.9 }, { cx: width + margin.right / 2, cy: 3.5, rx: 0.8, ry: 0.9 }];
g.selectAll(".ellipse").data(ellipses).enter().append("ellipse").attr("cx", function (d) {
    return d.cx;
}).attr("cy", function (d) {
    return y(d.cy);
}).attr("rx", function (d) {
    ...
    return y(d.ry);
}).style("stroke", "grey").style("stroke-width", "3");

// Add the ellipses anyons
// TODO: generate anyons from structure
g.selectAll(".anyons").data(links).enter().append("circle").attr("r", 6).attr("cx", function (d) {
    return -margin.left / 2;
}).attr("cy", function (d) {
    return y(d.id);
}).style("fill", function (d) {
    return d.chargeStart === "T" ? "dimgrey" : "white";
});
g.selectAll(".anyons").data(links).enter().append("circle").attr("r", 6).attr("cx", function (d) {
    ...
    return d.chargeEnd === "T" ? "dimgrey" : "white";
});
...
for (var step = 1; step < links[0].values.length; step++) {
    ...
    for (i = 0; i < slices.length; i++) {
        g.append("path").attr("class", "line").attr("class", "anyon" + slices[i][1].id).attr("d", function (d) {
            return line(slices[i]);
        }).style("stroke", function (d) {
            return z(slices[i][1].id);
        }).style("stroke-width", 4).on("mouseover", function (d) {
            svg.selectAll("." + this.getAttribute("class")).style("stroke-width", "6");
        }).on("mouseout", function (d) {
            svg.selectAll("." + this.getAttribute("class")).style("stroke-width", "4");
        });
    }
}
...
var sum = 0;
g.selectAll(".text").data(instructions).enter().append("text").attr("x", function (d) {
